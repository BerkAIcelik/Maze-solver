...
class MazeSolver:
    ...
    def handle_main_screen(self, events):
            ...
            if action:
                if action == "generate_maze":
                    ...
                    if self.maze:
                        ...
                        # Eğer seçili bir algoritma varsa, algoritmayı sıfırla ama başlatma
                        if self.ui.selected_algorithm:
                            self.current_algorithm = self.ui.get_selected_algorithm(self.maze)
                            if self.current_algorithm:
                                self.current_algorithm.initialize()
                                # main state'inde kal, çözüm algoritması çalışmasın
                                self.state = "main"
        ...
        if self.current_algorithm:
            ...
            # Eski mesaj gösterme kodunu kaldırıyorum
